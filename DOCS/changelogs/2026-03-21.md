...
- **FTXUI terminal dashboard** — complete TUI rewrite from raw printf/ANSI to FTXUI
  component-based rendering. Auto-resizing layout, proper containers, no more overlap
  bugs from manual cursor positioning.
...
- **Live sparkline graphs** — price and P&L history rendered as FTXUI graphs with
  auto-scaling and min/max labels. 120-point ring buffer (~2 min of data).

- **FoxML header banner** — fox ASCII art + FOXML block text logo from zsh welcome,
  rendered with the FoxML color palette (clay, wheat, mauve, sage).
...
### Fixed
- **Uptime display** — Widget_Header uses static start time instead of
  snapshot's zeroed start_time field.
